...
library(mda)
library(earth)
x_train <- train4 %>% select(-log_los_icu)
y_train <- train4 %>%  select(log_los_icu)



# default model
set.seed(1234)
mars1 <- earth(x=x_train, y=y_train, trace=3)
summary(mars1)
win.graph(h=7, w=6, pointsize=12)
plot(mars1)


prediction1 <- predict(mars1, newdata = test4)
mean((test4$log_los_icu - prediction1)^2) # 0.6489179


# model 2: allow 2-way interaction between explanatory variables
set.seed(1234)
mars2 <- earth(x=x_train, y=y_train, trace=3, degree = 2)
summary(mars2)
win.graph(h=7, w=6, pointsize=12)
plot(mars2)


prediction2 <- predict(mars2, newdata = test4)
mean((test4$log_los_icu - prediction2)^2) # 0.8882342

# CV: degree = 2 penalty = 3
set.seed(1234)
mars3 <- earth(x=x_train, y=y_train, trace=3, degree = 2, pmethod="cv", nfold=5) # penalty = 3
summary(mars3)
evimp(mars3)
mars3$gcv # GCV: 0.5389241
win.graph(h=7, w=6, pointsize=12)
plot(mars3) # saved as mars2.png


prediction3 <- predict(mars3, newdata = test4)
mean((test4$log_los_icu - prediction3)^2) # 0.8856526

# CV: degree = 1 penalty= 2
set.seed(1234)
mars4 <- earth(x=x_train, y=y_train, trace=3, degree = 1, pmethod="cv", nfold=5) # penalty = 2
summary(mars4)
evimp(mars4)
mars4$gcv # GCV: 0.5753701
win.graph(h=7, w=6, pointsize=12)
plot(mars4) # saved as mars1.png


prediction4 <- predict(mars4, newdata = test4)
mean((test4$log_los_icu - prediction4)^2) # 0.6242607 BEST MARS prediction used for ensemble method top layer
# write.csv(prediction4, file = "C:/Users/hyq92/Documents/R/844 Project/MIMIC III Data/Predictions/Pred_mars.csv")


prediction5 <- predict(mars4, newdata = train4)
# write.csv(prediction5, file = "C:/Users/hyq92/Documents/R/844 Project/MIMIC III Data/Predictions/Pred_mars2.csv")
